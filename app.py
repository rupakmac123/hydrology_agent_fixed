...
with st.expander("🌊 Scour Calculation (Tables 7, 8, 9)"):
    ...
    if st.session_state.results:
        ...
        with col1:
            ...
            L_bridge = st.number_input("Bridge Length (m)", value=226.17)
        ...
        with col3:
            ...
            freeboard = st.number_input("Freeboard (m)", value=1.5)
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            HFL_US = st.number_input("HFL US (m)", value=219.06)
            q_avg_US = st.number_input("q avg US (m²/s)", value=5.21)
        with col2:
            q_max_US = st.number_input("q max US (m²/s)", value=7.7)
        
        # Existing bridge section
        st.markdown("**Existing Bridge Section**")
        col1, col2, col3 = st.columns(3)
        with col1:
            HFL_EX = st.number_input("HFL EX (m)", value=218.6)
            q_avg_EX = st.number_input("q avg EX (m²/s)", value=5.28)
        with col2:
            q_max_EX = st.number_input("q max EX (m²/s)", value=8.1)
        ...
        if st.button("🔍 Calculate Scour Depths", type="primary"):
            ...
            st.success("✅ Scour calculation completed!")
    else:
